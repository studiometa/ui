...
- **`Slider`'s `mode: left|center|right` does not map onto scroll alignment.** Round 7 treated the two as equivalent. `Carousel.ts:163` hardcodes `align: SCROLL_ALIGNMENTS.center`, so a v1 slider that aligned left or right has nothing to migrate to. **Resolved: `mode` is not carried over.** v2 is a breaking release and the target is `Carousel`'s API as it stands, not `Slider`'s surface re-created on top of it. Adding an `align` option to hold a dropped component's behaviour would widen the API for a migration path rather than for a need. The migration guide states the loss plainly, and the option can be added later if a project asks for it.
