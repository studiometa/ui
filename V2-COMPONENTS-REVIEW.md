...
- **The spec is rewritten**, 12 tests to 21: the published properties, the damping, the `states` resolution and its clearing, the ancestor match, declaration-order precedence, the press behaviour, the diagnostic, and a browser assertion that a stylesheet `scale` does not move the cursor off the pointer. Tree total 940 → 949 across 86 files.
