...
import { AbstractPrefetch } from '#private/Prefetch/AbstractPrefetch.js';
import { PrefetchOnInteraction } from '#private/Prefetch/PrefetchOnInteraction.js';
import { PrefetchWhenVisible } from '#private/Prefetch/PrefetchWhenVisible.js';
...
describe('AbstractPrefetch — the hint', () => {
  ...
  it('appends one link for two components pointing at the same URL', async () => {
    ...
    const root = await mount(`
      ...
      <a data-component="AbstractPrefetch" href="${href}"></a>
    `);
    const [first, second] = [...root.children].map(
      (el) => getInstance<AbstractPrefetch>(el as HTMLElement, 'AbstractPrefetch')!,
    );
  ...
  it('still announces to the second component sharing the link', async () => {
    ...
    const root = await mount(`
      ...
      <a data-component="AbstractPrefetch" href="${href}"></a>
    `);
    const instances = [...root.children].map(
      (el) => getInstance<AbstractPrefetch>(el as HTMLElement, 'AbstractPrefetch')!,
    );
    let count = 0;
