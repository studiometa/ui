...
Because [`Action`](/reference/items/Action/) binds native listeners on its element, add both components to the same element and use the [`on:<event>` option](/reference/items/Action/js-api.html#on-event-modifier) to react to the emitted event:
