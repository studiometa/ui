...
### `update(url, requestInit, content, response)`
...
The same URL is what the [update events](../Fetch/js-api.md#event-order) describe, so `event.detail.request` carries the human-facing page on `fetch-update-before`, `fetch-update` and `fetch-update-after`, and the Section Rendering endpoint on the earlier events.
