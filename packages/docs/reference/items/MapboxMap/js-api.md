...
The base class for controls registered through `map.addControl`. It extends [`AbstractMapboxMapChild`](#abstractmapboxmapchild), creates the underlying Mapbox control lazily, and handles its add/remove lifecycle. Extend it when implementing a custom control component.
