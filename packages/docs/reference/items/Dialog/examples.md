...
Click the Open dialog button to open a modal wired entirely with [`Action`](/reference/items/Action/). The backdrop fades and the box scales in, each a [`Transition`](/reference/items/Transition/) child the `Dialog` orchestrates. Close it with the button, the backdrop, or <kbd>Esc</kbd>.
