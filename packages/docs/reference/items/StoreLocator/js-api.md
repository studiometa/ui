...
The store locator is an **orchestrator** built on the [`MapboxMap` family](/reference/items/MapboxMap/js-api):
...
A single store entry is a [`MapboxClusterItem`](/reference/items/MapboxMap/js-api#mapboxclusteritem). It registers with the closest `MapboxCluster`, exposes `id`, `lngLat`, `properties` and `popupContent`, and reflects its state through `setInBounds`/`setActive` (driven by the orchestrator). Refer to its [reference](/reference/items/MapboxMap/js-api#mapboxclusteritem) for the full API. The orchestrator itself never reintroduces a separate item class — it drives the cluster's items.
