...
The Twig template writes `data-component="Figure"`, and a component mounts on its configured name, so declare that name on a subclass:
...
- registerComponent(FigureTwicpics);
+ registerComponent(Figure);
```
...
By default, the `FigureTwicpics` component uses an blank SVG placeholder as the source of the image. This can be customized with the [`twic_placeholder` parameter](/reference/items/FigureTwicpics/twig-api.md#twic-placeholder).
