...
Use the [`offset` option](/reference/items/ScrollAnimation/js-api.html#offset) on `ScrollAnimationTimeline` to control when the animation starts and ends relative to the viewport.
...
You can easily implement a parallax effect with images by combining the `ScrollAnimationTarget` class with the [Figure component](/reference/items/Figure/).

Here, we use the [ImageGrid component](/reference/items/ImageGrid/) to quickly create the image layout. We are able to configure it to wrap each image in a `Parallax` component.
