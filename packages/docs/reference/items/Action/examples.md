...
A single `Action` can target several components at once. Here one button closes both a centered [`Dialog`](/reference/items/Dialog/) and a drawer by targeting every `Dialog` matching the `[data-can-be-closed]` attribute selector.
