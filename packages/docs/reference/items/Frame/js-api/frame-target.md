...
The `FrameTarget` component extends the [`Transition` component](/reference/items/Transition/) and inherits its APIs, with the exception of the [`enterKeep`](/reference/items/Transition/js-api.md#enterkeep) and [`leaveKeep`](/reference/items/Transition/js-api.md#leavekeep) options which are forced to always be `true`.
...
When using the `replace` mode, transitions defined by the [`Transition` component API](/reference/items/Transition/) will be played sequencially:
