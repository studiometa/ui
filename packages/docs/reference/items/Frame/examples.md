...
This example uses the [`Frame` components](/reference/items/Frame/), as well as the [`Action`](/reference/items/Action/), [`Figure`](/reference/items/Figure/) and [`Dialog`](/reference/items/Dialog/) components, to add support for adding products to a cart without reloading the page.
...
The `Dialog` component is used to display a nice cart as a drawer (see [Building a drawer](/reference/items/Dialog/#building-a-drawer)) and the `Action` component listens to the `Frame` event `frame-content`, which signals that the new content is being inserted, to open the cart and display the newly inserted content.
