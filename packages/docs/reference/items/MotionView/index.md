...
Containment is the wiring: with the [`auto` option](./js-api#auto) (on by default), a mounted `MotionView` wraps any `dom-update` announced inside its subtree — by [`Fetch`](/reference/items/Fetch/) or [`DataBind`](/reference/items/DataBind/)'s `data-bind:if` — and joins the extendable open/close lifecycle of a containing host, with zero wiring attributes on either side. See [ambient wiring](./js-api#ambient-wiring) — including why `@studiometa/ui`'s [`Dialog`](/reference/items/Dialog/) no longer takes part in it.
