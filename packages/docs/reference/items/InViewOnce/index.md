...
The `InViewOnce` primitive is a one-shot variant of the [`InView` primitive](/reference/items/InView/) that emits `in-view` once, when the element first enters the viewport, and never emits `out-of-view`.
