...
The `Timer` is a headless countdown primitive. It emits DOM events for its lifecycle — `timer-start`, `timer-end`, `timer-tick`, `timer-pause`, `timer-resume`, `timer-stop` — and exposes imperative methods, holding no UI state of its own. Combine it with [`Action`](/reference/items/Action/) to react to those events or call its methods, and with the [`Data*`](/reference/items/DataScope/) family to turn them into reactive state.
