...
export class DataRegistry {
  ...
  __groups = new Map<string, DataGroupRecord>();
  ...
  constructor({
    ...
    this.scoped = scoped;
    this.__defaultGroup = defaultGroup;
  ...
  /** @private */
  __record(group: string): DataGroupRecord {
    ...
    if (deletedKeys.length > 0 || updatedValues.size > 0) {
      ...
      for (const key of deletedKeys) {
        this.__notify(record, key, undefined);
      }
      for (const [key, value] of updatedValues) {
        this.__notify(record, key, value);
      }
    }
  ...
  __notify(
    ...
    this.__publishTo(record, { force: true, key, source: excludedSource, value });
  }

  /**
   * @private
   */
  __publishTo(record: DataGroupRecord, update: DataUpdate): DataUpdate {
  ...
  join(group: string, member: DataScopeMember): () => void {
    const record = this.__record(group);
  ...
  getData(group: string): Readonly<Record<string, DataValue>> {
    return this.__record(group).data;
  }
  ...
  deleteValue(group: string, key: string, source: DataScopeMember): void {
    ...
    if (sources.size === 0) {
      ...
      record.data = createSnapshot(record.values);
      this.__notify(record, key, undefined, source);
    } else if (group.endsWith('[]')) {
      const value = multipleSourcesValue(sources);
      record.values.set(key, value);
      record.data = createSnapshot(record.values);
      this.__notify(record, key, value, source);
    }
  }
