 ...
 *
 * **Continuous, not index-derived.** It reads the carousel's published scroll
 * progress, so the bar moves with the finger and stays correct under a peek or
 * a multi-slide layout, where `index / (total - 1)` is simply a different
 * quantity. It is also the exact value `animation-timeline: scroll()` takes
 * over natively once the CSS is shipped everywhere.
 *
 * **It measures nothing.** The offset is a percentage of the bar's own width,
 * so there is no layout read in the hot path and the vertical axis is the same
 * expression with the components swapped. The axis is read per update rather
 * than at mount, because `axis` is a responsive option: both components are
 * always written, so a carousel that turns vertical at a breakpoint unwinds
 * the horizontal transform on its own.
 *
 * ::: tip
...
> {
   ...
   *
   * A translate rather than a scale, which is why it needs no
   * `transform-origin`: put the bar in a container with
