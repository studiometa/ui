 ...
 *
 * It only reads the carousel, so it extends the non-subscribing base.
 *
...
export class CarouselDrag<T extends BaseProps = BaseProps> extends withDrag(
   ...
   *
   * The drag service announces its exact settle position at `drop`, so the
   * projected scroll offset is the pointer's own projected travel, mirrored.
   * Projecting the throw from the last event's delta instead would be a
   * per-device quantity, and the same flick would throw differently on a
   * 1000 Hz mouse and a 125 Hz trackpad.
   *
   ...
   * the intended result, not a defect — it is what makes a long carousel usable
   * with one gesture. A "one slide per flick" clamp was tried here and dropped:
   * it turns a throw into a step and takes the momentum out of the component.
   *
   ...
   * throw coasts to its projected position and stops wherever that is. That is
   * the track's own declaration rather than an option, because a component that
   * snapped a drop on a track the browser does not snap would be contradicting
   * the CSS.
   * @protected
   */
