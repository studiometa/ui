...
import { clamp, fold, wrap } from '@studiometa/js-toolkit/utils';
...
export class Indexable<T extends BaseProps = BaseProps> extends Base<IndexableProps & T> {
  ...
  get isReverse(): boolean {
    return this.__isReverse ?? this.$options.reverse === true;
  }
  ...
  get boundary(): IndexableBoundary {
    const boundary = this.__boundary ?? this.$options.boundary;
