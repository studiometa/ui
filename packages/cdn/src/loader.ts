import { registerComponent, type BaseConstructor } from '@studiometa/js-toolkit';
import type { ComponentLoadStrategy } from './component-metadata.js';
 ...
 *
 * This class is exported as a narrow test seam. The CDN's supported browser entry is `autoload.ts`.
 */
export class ComponentLoader {
  /** @private */
  __manifest: Record<string, ComponentManifestEntry>;
  /** @private */
  ...
  constructor({ manifest, dependencies }: ComponentLoaderOptions) {
  ...
  start({ eagerComponents = [] }: ComponentLoaderStartOptions = {}): void {
    ...
    if (MutationObserverConstructor) {
      ...
      this.__mutationObserver.observe(this.__dependencies.document, {
        childList: true,
        subtree: true,
      });
    } else {
    ...
    this.__scan(this.__dependencies.document);
  }
  ...
  __warn(message: string): void {
    this.__dependencies.console.warn(`${DIAGNOSTIC_PREFIX} ${message}`);
  }

  /** @private */
  __reportError(token: string, stage: 'import' | 'registration', error: unknown): void {
    this.__dependencies.console.error(
      `${DIAGNOSTIC_PREFIX} Failed during ${stage} for component "${token}".`,
      error,
    );
    const event = new this.__dependencies.CustomEvent('studiometa-ui:error', {
      bubbles: true,
